...
3. **ゲートを開く**: これは人間の行為であってエージェントの行為ではなく、開く場所は2つある。
   ...
   **これが立証していること、正確に。** 「人間が承認した」ことではない。リポジトリの中のどんな
   機構もそれは示せないし、このツールもそのふりはしない。receipt が記録するのは「確認が起きた
   こと」と「どのチャネルで」だけで、「**誰が**」までは残らない — 身元を束縛するモードは存在
   しない。成立しているのはもっと狭く、そして効いている主張だ — **承認は、事故でも、既定動作
   でも、誰かが事前承認した設定によっても起こらない。** これを支えるのは3つ。`rein approve` が
...
4. **修正を求める**: 成果物が正しくないときは、こちらが一級の答えになる — 行き止まりではない。
   ...
   解き、そのノートをあなたの承認画面に載せる — 承認がそれを閉じる。

   (修正要求はいかなる権限も要さない。起こることを**狭める**ことしかできないからだ。これが
   ダッシュボードの引く線で、狭める判断は書けて、広げる判断だけが起動リンクの守る対象になる。)
