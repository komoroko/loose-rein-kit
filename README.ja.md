...
`rein <verb> --help` が示す。エージェント自身が従うルールは [`AGENTS.md`](AGENTS.md) にある。
...
WSL・macOS に対応する。Windows native は**未検証**である —— WSL を使うこと。
...
つまみは `.rein/config.yaml` にある。既存リポジトリでは `rein init` が検出できた範囲を埋める。
...
| `execution.agent_timeout_sec` | 既定は `0`(制限なし)。時計は、動いているモデルと詰まっているモデルを区別できないため。command ステップは上限を持ったままである |
...
- **タスクが `awaiting-evidence` で止まっている** —— 受入基準の1つが `external`(ステージング確認・
  実機・人)である。作業はマージ済みで、誰かが見たものを `rein evidence record` で記録するまで待つ。
