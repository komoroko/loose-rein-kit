...
セットアップはこれで終わり。1つずつビルドする場合や、手で pin する場合は:
...
固定**する。以前は1ヶ月空けてビルドすると別のイメージができ、pin した digest は再現性の保証では
なく「たまたま焼けたもの」の記録でしかなかった。apt パッケージは今も Debian の生きたアーカイブを
見るため、かなり後のリビルドではまだずれ得る。digest の固定が保証するのは、承認済みレビューの
足元でインタプリタとベースファイルシステムが動かない点だけ。
...
3. **ゲートを開く**: `rein approve <gate>` は**人間が自分で**実行する — エージェントが打つ
   コマンドではないうえ、これがゲートを開ける唯一の手段。実行すると readiness を確認し、
   ...
   receipt が記録するのは「その端末で**誰か**が承認した」ことだけで、「**誰が**」までは残らない —
   身元を束縛するモードは存在しない。localhost のクリックは認証にならず、`--force` は存在せず、
