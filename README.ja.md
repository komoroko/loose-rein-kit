...
として渡る。置き場がユーザー設定であって `.rein/config.yaml` でないのは、後者が mandate によって凍結
されるためである。通知の宛先は mandate が凍結してよい対象ではない。

1つの判断につき通知は1回である。id は判断そのものが変わったときだけ変わる。通知が運ぶのは「何を待って
いるか」と「どこで答えるか」だけで、証拠も起動リンクも含まない。したがって機械の外へ出しても権限は
広がらない。チャネルが設定されているか、そのコマンドが実際に起動できるかは `rein doctor` が報告する。
