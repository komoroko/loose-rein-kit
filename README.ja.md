...
ハーネスの本体は**インストールして使う CLI**(`rein`)である。プロダクトのリポジトリ側に残るのは
*状態*だけで、`.rein/`(SSOT〈信頼できる唯一の情報源〉・lock・実体化された prompts/schema)と
...
Loose Rein はそれ自体が複数エージェントのオーケストレーションであり、次の3つの軸で設計している。
...
前提環境は POSIX 環境と、サンドボックス用のコンテナランタイム(docker/podman):
...
モード A(`rein build`)を使うには、加えて**ヘッドレスで動くエージェント CLI** が必要になる。
既定は `claude -p` で、`rein agent codex` で切り替えられる(`.rein/config.yaml` の
そのロールの adapter を `.rein/config.yaml` に設定する。`gemini` や任意のコマンドも指定できる)。用意できない場合は
...
リポジトリのコードとテストは、ホストではなく封じたOCIサンドボックスの中で走らせる
ためのものだ。ホストで走らせれば、エージェントが書いたテストファイルがあなたの資格情報つきで
実行される。Containerfile はツールに同梱され `.rein/oci/` に実体化されるので、レビューが
どの環境で走ったかは、それをビルドした wheel の中身ではなくリポジトリから監査できる:
...
セットアップはこれだけだ。1つずつビルドしたい場合や、手で pin したい場合は:

```bash
rein oci list                       # 同梱されている Containerfile の一覧
rein oci build --profile python     # ビルドして、pin すべき sha256: digest を表示する
```

`--profile` が取るのは **Containerfile の名前であって、executor profile の名前ではない**。
`quality` プロファイルは `python` からビルドされる — その `containerfile:` キーがそう書いている。
`--write-config` を付けなければ `oci build` は digest と貼るべきキー名を表示するだけだ。付けた
場合は、ビルドしたプロファイルの `kind`・`image`・`network_profile` の行だけを書き換える —
`config.yaml` のコメントは1つも失われず、パースできなくなるような書き込みは拒否する。pin は
gate ③ より前に済ませること。そこで config は凍結され、以降はガードが編集を拒否する。

Containerfile はベースイメージを `python:3.13-slim-bookworm` というタグではなく **digest で
固定**している。以前は1ヶ月空けてビルドすると別のイメージができていたので、pin した digest は
「再現できるもの」ではなく「たまたま焼けたもの」の記録でしかなかった。apt パッケージは今も
Debian の生きたアーカイブを見るため、かなり後のリビルドではまだずれ得る。digest の固定が保証
するのは、承認済みレビューの足元でインタプリタとベースファイルシステムが動かないことだ。

**digest が束縛するのは「どのイメージが走ったか」であって、「そのイメージがその手順を実行できるか」
ではない。** 同梱イメージが持つのは python・uv・pytest で、uid 1000・読み取り専用ルート・
`--network none` で走る。このリポジトリ自身がその隔たりの実例だ — 品質ゲートが `make test` と
`make check` を呼ぶのに `python` イメージには `make` が無く、`--network none` の下では
`uv run --frozen` が依存閉包に届かない。ここで pin すれば、通っていたゲートが落ちるようになる。
だからプロファイルは `kind: host` のまま出荷され、`doctor` はそれを `FAIL` として報告する。
解消するには、自分の DoD が必要とするツールチェーンと依存閉包を積んだ Containerfile を書く
ことになる。それは各リポジトリの判断であって、既定として誰かが配れるものではない。

## 自分で設定するリポジトリ側の項目

Loose Rein はこれらを読んで診断するだけで、設定はしない。ブランチ保護・必須チェック・シークレットは
リポジトリ管理の領域であり、自分を裁くチェックを自分で付け外しできるツールは境界にならない。
ホスティング側で一度だけ設定すること:
...
ローカルから見える部分だけは `rein doctor` が報告する——ワークフローが
`rein policy-check` を実行しているかどうか(していなければ WARN)。残りはリポジトリ管理者の
責務であり、`policy-check` を導入するコミット自体は自己検証されない(それを検査すべき、より古い
ベース側の検証器が存在しないため)。
...
3. **ゲートを開く**: `rein approve <gate>` は**人間が自分で**実行する — エージェントの
   コマンドではなく、ゲートを開ける唯一の手段でもある。実行すると readiness を確認し、
   この承認が束縛する digest を表示したうえで、確認入力を求める:
   ...
   receipt が記録するのは「その端末で**誰か**が承認した」ことだけで、「**誰が**」ではない —
   身元を束縛するモードは存在しない。localhost のクリックは認証ではなく、`--force` は存在せず、
   ゲート行の手編集はガードが拒否し、対話的な TTY が必須になる(パイプされた stdin・CI ジョブ・
   エージェントが捕捉したサブプロセスはプロンプトに答えられない)。
4. **差し戻す**: 上流(要件・設計)の不備が見つかったら `/revise <phase>` を実行する。戻し先以降の
   ゲートが連鎖的に `pending` へ戻り、影響を受けるタスクがマークされる
   (`rein revise --impacted T-00x` は、指定タスクとその下流をまとめて `needs-revision` に
   する)。承認の巻き戻しも人間の判断で行う。
5. **進捗を確認する**:
...
6. **PR にする**: `rein pr-draft` が SSOT から PR 本文を組み立てて `.rein/pr-draft.md`
   に書き出す(読み取り専用)。PR の作成や push は従来どおり人間の操作。
7. **サイクルを閉じる**: ゲート⑤のあと `rein cycle-close --name <slug>` を実行すると、docs が
...
- **タスクが `blocked` になった** — リトライ予算内で品質ゲートを通せなかったということ。
  `rein events --render` でエスカレーションの内容を読み、原因(またはタスク票)を直し、
  `state.yaml` の `status` を `todo` に戻してから `rein build` を再実行する。
  エスカレーションはログに残り続ける——追記専用で `resolve` に相当する動詞は無く、
  結論は `/verify` の回顧に書く。上流の不備が原因なら、代わりに `/revise <phase>`。
- **ループが中断した**(Ctrl-C・クラッシュ)— そのまま `rein build` を再実行すればよい。
