...
コメントは1つも失われず、パースできなくなる書き込みは拒否する。

**gate ③ の後でも、再 pin だけはできる。** gate ③ が凍結する `config.yaml` からは image の pin が
外してある。依存を正当に追加するタスクは pin 済みイメージを間違ったものにしてしまう — 必要な
依存が焼き込まれておらず、`network: none` のサンドボックスは何度リトライしても同じように落ちる —
のに、そのイメージを焼き直すには `rein revise --to tasks` が必要だった。計画が解凍され、下流の
ゲートが連鎖でリセットされ、人間が何も変わっていない計画を承認し直すことになる。いまは
`rein oci build --write-config` が pin を書き換え、`environment_repinned` イベントを追記し、
ゲートには触れない。ファイルのそれ以外は凍結されたまま — `kind`・`network_profile`・`mount_repo`・
品質ゲート・予算・ガード — なので、サンドボックスを開けるには、狭いほうを承認した人間が要る。
この許可の対価は可視性だ。`rein doctor` はサンドボックスが動いたことを報告し、gate ④ のオリエン
テーションは、承認者が署名しようとしている証拠が gate ③ の見ていない環境で作られたことを示す。

同梱の3つの Containerfile だけでは、リポジトリが抱えうる全ツールチェーンを賄いきれないこともある
— タスクがどれも想定していないツールチェーンに触れることはある。そのときはプロファイルに
`dockerfile:` — リポジトリ相対パス(例: `.rein/oci/custom/Containerfile`)、gate ③ で `config.yaml` の
...
6. **進捗を確認する**:
   ...
   - `rein ui` — ダッシュボードを開く。Overview ボードはこのキューを表示し、
     ...
     **orient**〈このサイクルが何を届けたか、依存とマイグレーションの変動、各品質ゲートが
     どのサンドボックスとネットワーク条件で走ったか、**この変更が人に何を要求するようになったか**
     — plan を見ていない reviewer がコードから読み取った運用面の事実を、gate③ でどのタスクかが
     宣言していたか否かで並べ替え、誰も宣言していないものを先頭に置く。変更「後」のファイルは
     1クリックで開ける — ゲートが確立したもの、まだ開いているもの(着地しなかったタスクについて
     実装者が述べたことを含む)、そして Expected/Actual の比較〉。
     すべて SSOT からの導出で、あなたには何も問わない。そのうえで **Decision Card**
     〈未決着の claim・gap・根拠なき追加挙動・security finding を1カード1件〉を、根拠を添えて
     提示する。high/critical のカードが未回答なら freeze はできない。Tasks タブ(DAG・レイヤー進行)、
...
- タスクは**品質ゲートのパイプラインをすべて通過して**はじめて完了になる。`config.yaml` の
  ...
  拒否する)。ステップには `paths:`(グロブパターン)を指定でき、タスクの差分がそれに触れているときだけ
  実行される — 複数の独立してテストできるスタックが混在するリポジトリで、無関係なスタックの分まで
  毎回コストを払わずに済む。これは gate ③ で凍結される config 側の判断であり、タスクの起票側が
  自分で加減できるつまみではない。`paths:` を指定しないステップは従来どおり全タスクで実行され、
  差分が確定できない場合(何も変更されていない新規 worktree など)は「何も対象がない」とは読まず、
  DoD をフルで実行する。
...
`rein build --supervise` はこのレシピをそのままプロセス内蔵にしたもの(意味論は同じ — `3`
だけを再試行し、各試行はそのときの `state.yaml` に対する新規 run):

```sh
rein build --supervise   # [--supervise-interval-sec N]、既定 900

# 間隔・バックオフを rein の外側で制御したい場合の等価な代替:
while :; do
  rein build && break
