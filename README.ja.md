...
**この境界が包むもの・包まないもの。** 包むのは、リポジトリ由来コードの**実行**である。すなわち
品質ゲートの `command` ステップと、同じランナーを通る受入基準の `command` である。包まないのは、
そのコードを**書いたエージェント CLI 自身**である。実装者・レビュアー・fixer は、いずれも `rein`
がホストプロセスとして、利用者のチェックアウト内で、利用者の資格情報のまま起動する。隔離があると
すれば、それはその CLI 自身が張るもの(`codex exec` は自前の seccomp/landlock サンドボックスを
持ち、`claude -p` はプロジェクトの権限設定に従う)に限られる。エージェント自体も包みたい場合は、
`rein` 自体をコンテナ内で動かし、アダプタ側のサンドボックスを無効化する。入れ子のサンドボックスは
エージェントが書き込む地点で失敗するため、`rein doctor` はその組み合わせを警告する。
...
- **環境も pin する。** リポジトリのコードとテストを*実行*する品質ゲートの command ステップは
  ...
  そのレビューの承認後に変わることはない。エージェント CLI 自身はホストプロセスであり、この境界の
  内側にはいない。
