...
def test_check_diff_accepts_a_flip_a_carry_after_the_rollback_backs(tmp_path: Path) -> None:
    ...
    about it alone — and its `gate_carried` names the mandate confirmation whose receipt opened it."""
    carried = ("gate_carried", ["acceptance", "GA-MANDATE-0002"])
    repo = _flip_fixture(tmp_path, events=_gate_events(_APPROVED, _REVISED, carried), approval_id="GA-MANDATE-0002")
    assert gate_guard.check_diff(repo) == 0
