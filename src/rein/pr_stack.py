...
(`gh stack link`). Landing the stack is not this module's: the harness takes a change to acceptance
and leaves it reviewable, and whoever owns the base merges it. What this module still owes that
person is the constraint the record depends on, printed where they will read it (`MERGE_NOTE`).
...
def slice_body(
    ...
    lines += [
        "",
        "Merge this stack whole, with merge commits — `gh stack merge` on the top pull request does "
        "it in one atomic operation. Do not merge part of it: GitHub rebases the pull requests above "
        "the cut with new commit ids, and the commit recorded for every task above it then exists in "
        "no branch's history. Squash and rebase merges strand them the same way. Landing it is "
        "yours, not the harness's: acceptance approved the change, not the push to the base.",
    ]
