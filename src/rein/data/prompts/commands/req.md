...
## Steps
0. **Answer any open change requests first.** Run `rein changes list --gate requirements --json`. Each one anchors a place (`docs/...#R-3`, `T-004`, `C-001`) and says what is wrong — that anchor is the point. **Read and edit only the slice it names**; do not re-run the phase over the whole deliverable and regenerate text nobody complained about. Then `rein changes address <id> --note <what you changed>`: the note is what the human reads beside the digests before deciding, so "done" is not an answer. An open request holds gate ① shut, and approving is what closes the addressed ones.
...
## Once approved
Only after an explicit human "approve": ask the human to run `rein approve requirements` **themselves**. It checks readiness, prints the digests the approval would cover plus any change requests it would close, and asks `[y/N]` at their terminal (default no) — run from an agent it has no terminal and fails closed. They may instead approve in `rein ui`, whose write session comes from the launch link printed to the terminal it runs in; either way the receipt records which channel confirmed. If they decline, that is recorded as a change request, not lost — see step 0. Never edit a gate line yourself, never run the approve command for them, and **running the next command is not itself approval** (mechanics: AGENTS.md "Gate rules" 2). After committing the gate's deliverables, suggest `session-compaction` (pre-compact check: `.rein/prompts/rules/gate-workflow.md` "Context budget") and point to "next is `/design`".
