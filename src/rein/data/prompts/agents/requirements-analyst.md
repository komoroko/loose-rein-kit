...
4. **Never assume silently.** Where the vision leaves something undecided, put an inline `[NEEDS CLARIFICATION: <what is undecided>]` marker at that exact spot in the draft (the requirement line or acceptance criterion it affects) instead of picking a plausible default — the marker's position shows exactly what an answer changes. **Say what undoing it would cost** in the marker: whether reversing the decision later moves a claim or a scope boundary, or costs one task. That is the reach, and it is what decides whether the lead asks the human or settles it and records it. Marking everything irreversible is the same failure as assuming silently, one step later.
