...
- **Codex's `exec` caps how long one command may run**, and a real build outlasts that cap. Start
  it detached with its output in a file (`nohup rein build --supervise > .rein/build.log 2>&1 &`),
  end the turn, and read `rein resume` / the log when you come back — **never re-run `rein build`
  to check on it**: the build lock makes the second run exit `3`, which is indistinguishable from a
  capacity stop (build.md, "When the run outlasts your host's command timeout").
