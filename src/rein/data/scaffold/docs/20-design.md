...
> Finalized once a human approves at **the mandate**.
...
> `.rein/plan.yaml`, and what acceptance holds the built code against. Writing "nothing" is a real
> answer; leaving them blank means acceptance meets whatever the code turned out to require with no
...
## Adversarial review
> Findings from the independent `adversarial-reviewer` round before the mandate (procedure: design.md step 6),
...
> Communicated to the human at the mandate as `.rein/prompts/rules/gate-workflow.md` "Gate self-assessment". Do not hide low-confidence design spots.
